...
die "Usage:perl $0 Gff.file seq.file\n" if(@ARGV==0);
...
check_redundance(\%mRNA);

mini_cds(\%mRNA,$Mini_cds) if (defined $Mini_cds);

perfect_gene(\%mRNA) if (defined $Perfect_gene);
	...
	while(<IN>){
		...
		if ($c[2] eq 'mRNA' && $c[8]=~/ID=([^;\s]+)/){
			...
			if ($Perfect_gene && $c[8]=~/Type=([^;\s]+)/){
				...
				die "You chose perfect_gene option ,but input gff file has no informatiob of type. Column 9 of mRNA  in  gff file  should be in fellow format:\n\tID=id;Type=type\nType must be:multi-exon,single-exon,no-first,no-last or no-first-last\n";
			}
...
sub check_redundance{
	...
	foreach my $seq_name (sort keys %$p) {
		...
		my ($num1,$num2,$num3);
                ($num1,$num2,$num3) = Conjoin_fragment(\@pos1);
                $all_size += $num1;
                $pure_size += $num2;
                $redunt_size += $num3;

                ($num1,$num2,$num3) = Conjoin_fragment(\@pos2);
                $all_size += $num1;
                $pure_size += $num2;
                $redunt_size += $num3;
	}
