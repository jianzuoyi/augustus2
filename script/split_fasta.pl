...
can not exceed the "file_in_dir" value, if it exceeds, then the result files 
...
my ($Attribute,$Cuts,$Cutf,$File_in_dir_num,$Pattern,$Unpattern,$Sample,$Fasta,$Prefix);
...
print STDERR "$Prefix";
if(!defined $Prefix && defined $Fasta){
        $Prefix = `basename $Fasta`;
#print STDERR "$Prefix---->$Fasta\n"£»
        chomp $Prefix;
}

get_attribute() if($Attribute);

cut_fasta($Fasta,$File_in_dir_num) if($Cuts);

cut_fasta_advanced($Fasta, $Prefix) if($Cutf); ##only one rank directory
...
#        chomp $file_name;
        my $out_dir = (defined $Outdir) ? $Outdir : "./$file_name.cut";
...
#       mkpath("$out_dir");
        ...
        if ($Cur_len>0 && $file_mark==$Cutf) {
                ##make the last file
                open OUT,">$out_dir/$file_name.$file_mark" || die "fail";
                print OUT $Cur_content;
                close OUT;
        }elsif($Cur_len>0 && $file_mark<$Cutf){
        my @last=split/>/,$Cur_content;
        for(my $j=$Cutf;$j>$file_mark;$j--){
            open OUT,">$out_dir/$file_name.$j" || die "fail";
            my $tmp=pop(@last);
            print OUT ">$tmp";
            close OUT;
        }
        $Cur_content=join">",@last;
        open OUT,">$out_dir/$file_name.$file_mark" || die "fail";
        print OUT $Cur_content;
        close OUT;
    }
